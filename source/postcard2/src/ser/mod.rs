...
/// ```
pub fn to_slice<'a, 'b, T>(value: &'b T, buf: &'a mut [u8]) -> Result<&'a mut [u8], SerializerError>
where
    T: Serialize + ?Sized,
{
...
pub fn to_vec<T>(value: &T) -> Result<alloc::vec::Vec<u8>, SerializerError>
where
    T: Serialize + ?Sized,
{
...
/// ```
pub fn to_extend<T, W>(value: &T, writer: W) -> Result<W, SerializerError>
where
    T: Serialize + ?Sized,
...
/// ```
#[cfg(feature = "std")]
pub fn to_io<T, W>(value: &T, writer: W) -> Result<W, SerializerError>
where
    T: Serialize + ?Sized,
    W: std::io::Write,
{
...
pub fn serialize_with_flavor<T, S>(value: &T, storage: S) -> Result<S::Output, SerializerError>
where
    T: Serialize + ?Sized,
    S: Flavor,
{
...
pub fn serialized_size<T>(value: &T) -> Result<usize, SerializerError>
where
    T: Serialize + ?Sized,
{

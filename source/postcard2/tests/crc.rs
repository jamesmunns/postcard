...
fn test_crc() {
    ...
    let res = postcard2::to_slice_crc32(data, buffer, digest).unwrap();
    assert_eq!(res, &[0x04, 0x01, 0x00, 0x20, 0x30, 0x8E, 0xC8, 0x1A, 0x37]);

    let digest = crc.digest();
    let res = postcard2::take_from_bytes_crc32::<[u8; 5]>(res, digest).unwrap();
...
fn test_crc_8() {
    ...
    let digest = crc.digest();
    let res = postcard2::ser_flavors::crc::to_slice_u8(data, buffer, digest).unwrap();
    assert_eq!(res, &[0x04, 0x01, 0x00, 0x20, 0x30, 167]);

    let digest = crc.digest();
    let res = postcard2::de_flavors::crc::take_from_bytes_u8::<[u8; 5]>(res, digest).unwrap();
...
fn test_crc_error() {
    ...
    let digest = crc.digest();
    let res = postcard2::take_from_bytes_crc32::<[u8; 5]>(res, digest);

    assert_eq!(res, Err(postcard2::Error::DeserializeBadCrc));
}
...
fn test_crc_in_method() {
    use crc::{Crc, CRC_32_ISCSI};
    use postcard2::{to_slice_crc32, Result};
    use serde::Serialize;
